...
const createCommunity = async (req: any, res: any) => {
  ...
  try {
    ...
    const data: QueryResult<any> = await client.query(
      communityQuery,
      communityParams
    );
...
const myOwnedCommunity = async (req: ReqMid, res: any) => {
  ...
  try {
    ...
    res
      ...
      .json({
        status: true,
        content: { meta, data: communities },
        message: "Retrived all the communities owned by the user",
      });
  } catch (error) {
    console.log(error);
  ...
  try {
    ...
    res
      ...
      .json({
        status: true,
        content: { meta, data: communities },
        message: "Retrived user joined communities",
      });
  } catch (error) {
    console.log(error);
    res.status(500).json({ status: false, message: "Internal Server Error" });
